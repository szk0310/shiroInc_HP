...
if ($recaptchaData->success) {
	...
	$url = 'https://webto.salesforce.com/servlet/servlet.WebToLead?orgId=00D2r000000Eqa0';
	$data = [];

	// $dataに送るデータを詰めます。
	$data['company'] = trim($_REQUEST['company']);
	$data['last_name'] = trim($_REQUEST['name']);
	$data['email'] = trim($_REQUEST['email']);
	$data['oid'] = "00D2r000000Eqa0";
	// 送信データをURLエンコード
	...
	header('Location:https://go.shiro-k.jp/HP/thanks');
	exit();
} else {
	// reCAPTCHAが失敗した場合の処理
	header('Location:https://go.shiro-k.jp/HP/error');
	exit();
